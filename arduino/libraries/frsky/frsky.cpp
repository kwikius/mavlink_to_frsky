...
namespace {
 ...
 //  inline int32_t normalise_angle(quan::angle_<int32_t>::deg10e7 const & in)
   inline int32_t normalise_angle(int32_t const & in)
   {
   ...
   */
      ...
      for( ; pos < len; ++pos){
          ...
          if ( (ch == FrSky_msgID::header_value) || (ch == FrSky_msgID::escape_value) ){
           ...
           //  frsky_serial_port::write(ar,2);
             asynch_tx_write(ar,2);
             count += 2;
          }else{
            // frsky_serial_port::write(buf + pos,1);
             asynch_tx_write(buf + pos,1);
             ++count;
          }
      }
      return count;
   ...
   int16_t update_baroalt_msg1()
   {
     // baroalt_msg = static_cast<int32_t>(the_aircraft.location.baro_alt.numeric_value());
      baroalt_msg = static_cast<int32_t>(the_aircraft.location.baro_alt);
   ...
   int16_t update_airspeed_msg1()
   {
     ...
     airspeed_msg = static_cast<int32_t>(the_aircraft.airspeed);
